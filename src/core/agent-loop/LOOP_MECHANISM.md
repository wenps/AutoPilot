...
- **仅在本轮工具全部失败或无成功 DOM 变更时才累计计数**。
- 若本轮有成功的 DOM 变更（click/fill 等），视为实质推进，不累计协议缺失。
- 目的：兼容不遵循 REMAINING 协议的模型（如 DeepSeek），避免因协议不合规而过早终止正在推进的任务。
